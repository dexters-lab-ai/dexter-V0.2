...
export class WalletDetailsHandler {
  ...
  async showWalletDetails(chatId, userInfo, address) {
    ...
    try {
      ...
      // Fetch token balances based on network
      let tokenBalances = [];
      ...
      // ✅ Prevent Telegram "message too old" error
      if (loadingMsg && loadingMsg.message_id) {
        try {
          await this.bot.deleteMessage(chatId, loadingMsg.message_id);
        } catch (deleteError) {
          console.warn("⚠️ Couldn't delete loading message, possible timeout:", deleteError);
        }
      }
